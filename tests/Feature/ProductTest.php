...
class ProductTest extends TestCase
{
    ...
    public function test_that_a_product_can_be_found()
    {
        $this->withoutExceptionHandling();
        ...
        $productFindResponse = $this->withHeaders(
            [
                'Authorization' => 'Bearer '.$login['token'],
                'Accept' => 'application/json'
            ]
        )->get('/api/products/'.$productCreatedResponse['id'], []);

        $productFindResponse->assertStatus(200);
        $this->assertTrue($productFindResponse['name'] === 'IPhone 12 Pro');
    }
}
